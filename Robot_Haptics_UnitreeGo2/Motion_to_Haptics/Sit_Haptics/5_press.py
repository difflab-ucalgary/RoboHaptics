...
class Go2Leg:
    ...
    def fr_press_ten(self,
                 contact_q  = [0.0, 0.0, -0.60],
                 hold_s=3.0,
                 finished_q=[0.0, 1.5, -2.75],
                 approach_q=[0.0, 0.0, -2.50]):
        print("[INFO] Performing FR press...")
        ...
        for i in range(10):
            # approach slowly
            self._move_fr_to(approach_q, t_move=2, kp=50.0, kd=3.0)
            self._hold_current_Pos(q_target = approach_q, t_lock_time=2.0)
            # press down firmly to contact
            self._move_fr_to(contact_q, t_move=3.0, kp=65.0, kd=5.0)

            # hold pressure (higher stiffness)
            t0 = time.time()
            while time.time() - t0 < hold_s:
                ...
                self._lock_non_fr(kp=80, kd=5.0)
                self.send_cmd()
                time.sleep(0.01)

        # retreat to finish
        self._move_fr_to(finished_q, t_move=1.0, kp=55.0, kd=4.0)
        print("[INFO] Press complete.")
