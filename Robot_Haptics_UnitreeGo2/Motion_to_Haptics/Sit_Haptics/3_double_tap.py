...
class Go2Leg:
    ...
    def f_double_tap_ten(self,
                      approach_q = [0.0, 0.0, -2.75],
                      contact_q  = [0.0, 0.0, -0.90],
                      middle_q  = [0.0, 0.0, -1.5],
                      finished_q = [0.0, 1.5, -2.75],
                      approach_t = 0.6, hold_s = 0.5, retreat_t = 0.6):
        """
        Perform a tap with the front-right leg (FR).
        Sequence: approach -> tap -> retreat.
        Other legs are locked for stability.
        """
        print("[INFO] Starting FR tap sequence...")
        ...
        for i in range(10):
            # Step 1: approach
            self._move_fr_to(middle_q, t_move=approach_t, kp=55.0, kd=3.0)
            self._hold_current_Pos(q_target = self._sitonFootPos1, t_lock_time=2.0)
            ...
            t0 = time.time()
            while time.time() - t0 < 0.15:   # hold 150ms
                for j in range(3):
                    i = j
                    self.cmd.motor_cmd[i].mode = 0x01
                    self.cmd.motor_cmd[i].q    = [0.0, 0.0, -0.9][j]
                    self.cmd.motor_cmd[i].kp   = 65.0
                    self.cmd.motor_cmd[i].kd   = 5.0
                    self.cmd.motor_cmd[i].dq   = 0.0
                    self.cmd.motor_cmd[i].tau  = 0.0
                self._lock_non_fr(kp=65.0, kd=5.0)
                self.send_cmd()
                time.sleep(0.01)
            
            # Step 1: approach
            self._move_fr_to(middle_q, t_move=0.5, kp=55.0, kd=3.0)
        ...
        # Step 4: retreat back
        self._move_fr_to([0.0, 0.0, -2.0], t_move=0.6, kp=55.0, kd=4.0)
        # finish position
        self._move_fr_to(finished_q, t_move=0.6, kp=60.0, kd=5.0)
