...
class Go2Leg:
    ...
    def fr_touch_ten(self,
                 contact_q  = [0.0, 0.0, -0.90],
                 ready_q = [0, 0, -2.75],
                 hold_s=2.0,
                 finished_q=[0.0, 1.5, -2.75]):
        print("[INFO] Performing FR light touch...")
        ...
        for i in range(10):
            # move FR gently to contact
            self._move_fr_to(ready_q, t_move=0.5, kp=60.0, kd=5.0)
            self._hold_current_Pos(q_target = ready_q, t_lock_time=2.0)

            self._move_fr_to(contact_q, t_move=1.75, kp=25.0, kd=2.0)
            ...
            while time.time() - t0 < hold_s:
                for j in range(3):
                    i = j
                    self.cmd.motor_cmd[i].mode = 0x01
                    self.cmd.motor_cmd[i].q = contact_q[j]
        ...
        # retreat to finished
        self._move_fr_to(finished_q, t_move=1.0, kp=30.0, kd=3.0)
        print("[INFO] Touch complete.")
